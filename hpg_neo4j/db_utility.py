...
# def APIv2_build_property_graph_for_files(input_files, output_directory, output_mode='csv', graphid=None, timeout=30*60):

# 	"""
# 	builds an HPGv2 (nodes.csv, rels.csv) for a given set of input files 
# 	"""

# 	if graphid is None:
# 		graphid = generate_uuid_for_graph()

# 	input_files_arg = "--input=" +  " --input=".join(input_files)

# 	command = "node --max-old-space-size=32000 {0} --lang=js --graphid={1} --mode={2} {3} --output={4}".format(constantsModule.ANALYZER_DRIVER_PATH_V2, graphid, output_mode, input_files_arg, output_directory)
...
# def API_build_property_graph(site_id, site_name, url, print_debug=constantsModule.DEBUG_PRINTS):

# 	"""
# 	builds a property graph database in the CSV format
# 	"""

# 	folder_name_of_url = site_name + "_" + _hash(url)
